...
async function renderPicksSection() {
  ...
  container.innerHTML = weeklyPicks.map(function(pick) {
    ...
    return `
      <div class="pick-card-covers">
        <div class="pick-covers-header">
          <div class="pick-covers-meta">
            <span class="pick-sport-badge">${escapeHtml(pick.sport)}</span>
            ${postedDate ? `<span class="pick-covers-date">${escapeHtml(postedDate)}</span>` : ''}
          </div>
          <span class="result-badge ${resultCls}">${escapeHtml(pick.status || 'Pending')}</span>
        </div>
        <div class="pick-covers-matchup">${escapeHtml(pick.matchup)}</div>
        <div class="pick-covers-recommendation">
          <span class="pick-covers-label">Pick</span>
          <span class="pick-covers-value">🏆 ${escapeHtml(pick.pickDetails)}</span>
          <span class="pick-covers-type">${escapeHtml(pick.pickType)}</span>
        </div>
